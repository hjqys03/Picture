...
const _tmpl$$1 = /*#__PURE__*/template(\`<h2>🥳 ComicRead 已更新到 v\`),
  ...
  _tmpl$4 = /*#__PURE__*/template(\`<h3>修复\`),
  _tmpl$5 = /*#__PURE__*/template(\`<ul><li><p>修复 Ehentai 无法加载图片的问题</p></li><li><p>修复 拷贝漫画 加载图片缓慢的问题</p></li><li><p>修复 Anchira 无法加载图片的问题\`);
...
const main = require('main');
    ...
    // 移除 E-Hentai 自带的「多页查看器」按钮
    main.waitDom('.g2.gsp').then(() => {
      document.querySelectorAll('.g2.gsp').forEach(el => {
        if (el.textContent.includes('多页查看器') || el.textContent.includes('Multi-Page Viewer')) {
          el.remove();
        }
      });
    });
...
(async () => {
  ...
  /**main.insertNode(document.getElementById('gd5'), '<a id="comicReadMode" href="javascript:;"></a>');*/
