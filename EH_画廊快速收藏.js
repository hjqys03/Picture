...
// @version      v2.2.2
// @description  增强画廊收藏按钮，允许快速将画廊加入收藏夹，并支持备注功能。
...
function getGMColors() {
    ...
    let gmStyle = gm ? window.getComputedStyle(gm) : null;
    return {
        border: bodyStyle.color, // 边框颜色跟随 body 的字体颜色
        bg: gmStyle ? gmStyle.backgroundColor : bodyStyle.backgroundColor,
        bg_light: gmStyle ? gmStyle.backgroundColor : bodyStyle.backgroundColor
    };
}
...
// 调整颜色亮度 (amt > 0 变亮, amt < 0 变暗)
function adjustColor(col, amt) {
    let c = document.createElement('div');
    c.style.color = col;
    document.body.appendChild(c);
    let rgb = window.getComputedStyle(c).color;
    document.body.removeChild(c);
    let m = rgb.match(/\d+/g);
    let r = parseInt(m[0]) + amt;
    let g = parseInt(m[1]) + amt;
    let b = parseInt(m[2]) + amt;
...
// hover 背景色
var hoverBg = adjustColor(color.bg, 20);
...
#gdf > .favnote:after             { width: 50px; content: "查看备注"; }
#gdf > .favnote.plus:after        { width: 50px; content: "添加备注"; }
#gdf > .favnote.arrow:after       { width: 50px; content: "保存备注"; }
#gdf > .favnote.pencil:after      { width: 50px; content: "编辑备注"; }
#gdf > .favnote.exclamation:after { width: 300px; height: 60px !important; white-space: normal; content: "备注加载失败。点击重试。⚠️ 注意：在备注未加载时更改收藏类别会删除已有备注！"; }
...
function injectQFElements() {
    ...
    <div qfid='favdel' class='fav'>
        <a id="favoritelink" href="#">从收藏中移除</a>
...
function updateCurrentFav(favID) {
    ...
    let prot = location.protocol;
    let host = location.host;
    let favUrl = `${prot}//${host}/gallerypopups.php?gid=${galID}&t=${token}&act=addfav`;
    ...
    if (favID == "favdel" || favID === 10) {
        ...
        <div style="float:left">
            &nbsp; <a href="#" onclick="window.open('${favUrl}', 'favwin', 'width=675,height=415,resizable=yes,scrollbars=yes'); return false;" id="favoritelink">
                <img src="https://ehgt.org/g/mr.gif"> Add to Favorites
            </a>
            <div class="hotkey-hint"></div>
        </div>
        `;
        ...
        <div style="float:left">&nbsp;
            <a href="#" onclick="window.open('${favUrl}', 'favwin', 'width=675,height=415,resizable=yes,scrollbars=yes'); return false;" id="favoritelink">${favLabels[favID]}</a>
        </div>
        `;
    }
