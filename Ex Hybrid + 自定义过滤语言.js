        ...
        if (!seriesSearchEnabled) {
          ...
          if (!isAnthology) {
            ...
            cleanTitle = extractTitle
              ...
              // ✅ 遇到【】、「」、『』、＜＞、<>、（）、()、《》 任意一种括号后，截断后面所有内容
              .replace(/[【「『＜《<(（(][^【】「」『』＜＞《》<>（）()]+[】」』＞》>)）)].*$/g, "")
              // ✅ 删除冒号（:、：、-、~、～、—、〜、―、﹣）及其后的内容，或删除出现成对连接符号（例如 ～ ～、― ―）及其中间内容
              .replace(/[:：\-~～—〜―﹣].*$|([\-~～—〜―ー﹣]).*?\1.*$/i, "")
              // ✅ 删除以 "#" 开头的章节编号或数字后紧跟空格的部分
              .replace(/#\d+(?:\.\d+)?\s*.*$|\d+(?:\.\d+)?\s.*$/i, "")
              ...
              // ✅ 去掉末尾的各种章节/卷号标识
              .replace(/\s*(?:前編|中編|後編|前篇|中篇|後篇|前章|中章|後章|最終編|最終篇|最終話|最終章|最終巻|最終卷|完全版|上巻|中巻|下巻|上卷|中卷|下卷|総集編|総集篇|総集章|単行本版|After(?:[上下中前後]?(?:巻|卷|篇|編|章)?)?)\s*$/i, "")
