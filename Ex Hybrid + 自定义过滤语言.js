        ...
        if (!seriesSearchEnabled) {
          ...
          if (!isAnthology) {
            ...
            cleanTitle = extractTitle
              ...
              .replace(/\s*(?:第?\d+(?:\.\d+)?[\-~～—〜―﹣+]\d+(?:\.\d+)?(?:話|巻|卷|篇|編|章)?|第\d+(?:\.\d+)?(?:話|巻|卷|篇|編|章)?|Vol\.?\s*(?:\d+(?:\.\d+)?(?:[\-~～—〜+]\d+(?:\.\d+)?)?|[IVXⅰⅴⅵⅶⅷⅸⅹ]+(?:[\-~～—〜+][IVXⅰⅴⅵⅶⅷⅸⅹ]+)?)|\d+(?:\.\d+)?[\-~～—〜―﹣+]\d+(?:\.\d+)?(?:話|巻|卷|篇|編|章)?|第?\d+(?:\.\d+)?\s*(?:巻|卷|話|篇|編|章)?|[IVXⅰⅴⅵⅶⅷⅸⅹ]+(?:[\-~～—〜―﹣+][IVXⅰⅴⅵⅶⅷⅸⅹ]+)?|[上下中前後最終]?章|[一二三四五六七八九十]+(?:章|話|巻|卷|篇|編))\s*/gi, "")
              // ✅ 去掉末尾的各种章节/卷号标识
              .replace(/\s*(?:前編|中編|後編|前篇|中篇|後篇|前章|中章|後章|最終編|最終篇|最終話|最終章|最終巻|最終卷|完全版|上巻|中巻|下巻|上卷|中卷|下卷|総集編|総集篇|総集章|単行本版(?:[上下中前後]?(?:巻|卷|篇|編|章)?)?)\s*/gi, "")
