        ...
        if (!seriesSearchEnabled) {
          ...
          if (!isAnthology) {
            ...
            cleanTitle = extractTitle
              // ✅ 删除 # 后紧跟数字或小数及其后面的所有内容
              .replace(/#\d+(?:\.\d+)?\s*.*$/, "")
              // ✅ 删除「」或“”里的内容
              .replace(/[「『][^「『」』]+[」』]/g, "")
              // ✅ ":"、"：" 一旦出现立即截断、连接符 (-、~、～、—、〜、―、ー、﹣) 仅当成对（相同符号）出现时才清理
              .replace(/[:：].*$|([\-~～—〜―ー﹣]).*?\1.*$/, "")
              // ✅ 去掉带数字或罗马数字的卷号/章节号（含小数点版本）
              .replace(/\s*(?:第?\d+(?:\.\d+)?[\-~～—〜―ー﹣+]\d+(?:\.\d+)?(?:話|巻|卷|篇|編|章)?|第\d+(?:\.\d+)?(?:話|巻|卷|篇|編|章)?|Vol\.?\s*(?:\d+(?:\.\d+)?(?:[\-~～—〜―ー﹣+]\d+(?:\.\d+)?)?|[IVXⅰⅴⅵⅶⅷⅸⅹ]+(?:[\-~～—〜―ー﹣+][IVXⅰⅴⅵⅶⅷⅸⅹ]+)?)|\d+(?:\.\d+)?[\-~～—〜―ー﹣+]\d+(?:\.\d+)?(?:話|巻|卷|篇|編|章)?|第?\d+(?:\.\d+)?\s*(?:巻|卷|話|篇|編|章)?|[IVXⅰⅴⅵⅶⅷⅸⅹ]+(?:[\-~～—〜―ー﹣+][IVXⅰⅴⅵⅶⅷⅸⅹ]+)?)\s*$/i, "")
              // ✅ 去掉末尾的各种章节/卷号标识
              .replace(/\s*(?:前編|中編|後編|前篇|中篇|後篇|前章|中章|後章|最終編|最終篇|最終話|最終章|最終巻|最終卷|完全版|上巻|中巻|下巻|上卷|中卷|下卷|総集編|総集篇|総集章|単行本版(?:[上下中前後]?(?:巻|卷|篇|編|章)?)?)\s*$/i, "")
