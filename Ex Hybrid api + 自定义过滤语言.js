  ...
  const colorSyncTimer = setInterval(() => {
    ...
    if (lastSort.key && lastSort.dir && lastSort.dir !== "none") {
      ...
      function renderRows(listToRender) {
        ...
        const languageFlags = {
          ...
          "text cleaned": "data:image/svg+xml;base64,",
        ...
        listToRender.forEach((g) => {
          ...
          if (!flagKey) flagKey = "text cleaned";
        ...
        popup.querySelectorAll(".bt-copy-button").forEach(btn => {
          btn.onclick = (e) => {
            const oldUrl = btn.dataset.url; // 原始链接
            const match = oldUrl.match(/\/g\/(\d+)\/([a-z0-9]+)/i);
            if (!match) return showToast("❌ 链接解析失败");
            const gid = match[1];
            const token = match[2];
            const host = location.hostname; // 当前域名
            const newLink = `https://${host}/g/${gid}/${token}`;
            navigator.clipboard.writeText(newLink).then(() => showToast("✅ 已复制链接"));
          };
        });
    ...
    async function fetchSimilarList() {
      ...
      try {
        ...
        if (!seriesSearchEnabled) {
          ...
          if (!isAnthology) {
            ...
            cleanTitle = extractTitle
              ...
              .replace(/#\d+(?:\.\d+)?\s*.*$|(?<!Vol\.)\b(?:\d+(?:\.\d+)?|(?<![A-Z])[IVXⅰⅴⅵⅶⅷⅸⅹ](?![A-Z])|[①-⑳壱弐参肆伍陸柒捌玖拾]+)(?:\s.*|$)/i, "")
              ...
              // ✅ 去掉末尾的各种章节/卷号标识
              .replace(/\s*(?:前編|中編|後編|前篇|中篇|後篇|前章|中章|後章|前日譚|後日譚|最終編|最終篇|最終話|最終章|最終巻|最終卷|完全版|上巻|中巻|下巻|上卷|中卷|下卷|総集編|総集篇|総集章|単行本版|フルカラー|After(?:[上下中前後]?(?:巻|卷|篇|編|章)?)?)\s*$/i, "")
              // ✅ 去掉末尾的各种章节
              .replace(/\s*(?:前編|中編|後編|前篇|中篇|後篇|前章|中章|後章|前日譚|後日譚|最終編|最終篇|最終話|最終章|最終巻|最終卷|完全版|上巻|中巻|下巻|上卷|中卷|下卷|総集編|総集篇|総集章|単行本版|フルカラー|After)\s*$/i, "")
              // ✅ 去掉开头或结尾的「続」「続・」「・続」「続編」「続章」
              .replace(/^(?:続(?:[・･·•])?|[・･·•]?続(?:話|巻|卷|篇|編|章)?)\s*|\s*(?:続(?:[・･·•])?|[・･·•]?続(?:話|巻|卷|篇|編|章)?)$/gi, "")
              // ✅ 修改：全局匹配第一話、第一章、第一巻、第一卷、第一篇、第一編，不限定末尾
              .replace(/第[一二三四五六七八九十壱弐参肆伍陸柒捌玖拾]+(?:話|章|巻|卷|篇|編)/gi, "")
        ...
        // 使用 E-Hentai API 一次性获取所有画廊信息
        const gidList = [];

        for (const item of list) {
          const match = item.url.match(/\/g\/(\d+)\/([0-9a-fA-F]+)\//);
          if (match) gidList.push([match[1], match[2]]);
        }

        // 分批请求（API 每次最多支持 25 个）
        const allMeta = [];
        const chunkSize = 25;
        ...
        for (let i = 0; i < gidList.length; i += chunkSize) {
          ...
          try {
            ...
            console.warn("📡 API 请求失败：", err);
          }

          await new Promise(r => setTimeout(r, 250)); // 防止请求过快被限流
        }

        const currentHost = location.hostname; // 当前页面的域名：exhentai.org 或 e-hentai.org

        // 把 API 返回结果映射回原来的列表结构
        const gidMap = {};
        for (const g of allMeta) {
          gidMap[g.gid] = {
            title: g.title_jpn || g.title || "",
            engTitle: g.title || "",
            cover: g.thumb,
            language: (() => {
              const langTag = g.tags?.find(t => t.startsWith("language:") && !/translated|rewrite/i.test(t));
            ...
            posted: g.posted
              ...
              : "—",
            url: `https://${currentHost}/g/${g.gid}/${g.token}/` // 🔹使用当前域名
          };
        }

        // 更新缓存列表
        cachedList = list.map(item => {
          const match = item.url.match(/\/g\/(\d+)\/([0-9a-fA-F]+)\//);
          const gid = match ? match[1] : null;
          return gidMap[gid] || item;
        });
          ...
          if (isManga && bracketMatch) {
            ...
            if (extraKeyword) {
              ...
              if (!tempList.length) {
                ...
                // ===== 改为使用 E-Hentai API 获取附加搜索详情 =====
                const gidListExtra = [];

                for (const item of tempList) {
                  const match = item.url.match(/\/g\/(\d+)\/([0-9a-fA-F]+)\//);
                  if (match) gidListExtra.push([match[1], match[2]]);
                }

                const allMetaExtra = [];
                const chunkSizeExtra = 25;
                ...
                for (let i = 0; i < gidListExtra.length; i += chunkSizeExtra) {
                  ...
                  try {
                    ...
                    console.warn("📡 附加搜索 API 请求失败：", err);
                  }

                  await new Promise(r => setTimeout(r, 250)); // 防止触发限速
                }

                const gidMapExtra = {};
                for (const g of allMetaExtra) {
                  gidMapExtra[g.gid] = {
                    title: g.title_jpn || g.title || "",
                    engTitle: g.title || "",
                    cover: g.thumb,
                ...
                let extraList = tempList.map(item => {
                  const match = item.url.match(/\/g\/(\d+)\/([0-9a-fA-F]+)\//);
                  const gid = match ? match[1] : null;
                  return gidMapExtra[gid] || item;
                });
