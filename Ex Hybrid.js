        ...
        if (extractTitle) {
          if (!isAnthology) {
            ...
            cleanTitle = extractTitle
              ...
              .replace(/【[^【】]+】/g, "")
              // ✅ 去掉末尾的各种章节/卷号标识
              .replace(/\s*(?:前編|中編|後編|最終編|最終話|最終巻|最終卷|上巻|中巻|下巻|上卷|中卷|下卷|総集編|単行本版(?:[・･·•]?[上下中]?(?:巻|卷)?)?|第?\d+[\-~～—〜―﹣+]\d+(?:話|巻|卷)?|第\d+話|Vol\.?\s*(?:\d+(?:[\-~～—〜+]\d+)?|[IVXⅰⅴⅵⅶⅷⅸⅹ]+(?:[\-~～—〜+][IVXⅰⅴⅵⅶⅷⅸⅹ]+)?)|\d+[\-~～—〜―﹣+]\d+(?:話|巻|卷)?|第?\d+\s*(?:巻|卷|話|編)?|[IVXⅰⅴⅵⅶⅷⅸⅹ]+(?:[\-~～—〜―﹣+][IVXⅰⅴⅵⅶⅷⅸⅹ]+)?)\s*$/i, "")
              // ✅ 仅在末尾去除中点符号
              .replace(/[・･·•]+$/g, "")
              .trim();
          } else {
