        ...
        if (extractTitle) {
          if (!isAnthology) {
            ...
            cleanTitle = extractTitle
              ...
              // ✅ 去掉被【】包裹的说明
              .replace(/【[^【】]+】/g, "")
              // ✅ 去掉末尾的各种章节/卷号标识（全面支持「編 / 篇 / 巻 / 卷 / 話」等）
              .replace(/\s*(?:前編|中編|後編|前篇|中篇|後篇|最終編|最終篇|最終話|最終巻|最終卷|上巻|中巻|下巻|上卷|中卷|下卷|総集編|総集篇|単行本版(?:[・･·•]?[上下中]?(?:巻|卷|篇|編)?)?|第?\d+[\-~～—〜―﹣+]\d+(?:話|巻|卷|篇|編)?|第\d+(?:話|巻|卷|篇|編)|Vol\.?\s*(?:\d+(?:[\-~～—〜+]\d+)?|[IVXⅰⅴⅵⅶⅷⅸⅹ]+(?:[\-~～—〜+][IVXⅰⅴⅵⅶⅷⅸⅹ]+)?)|\d+[\-~～—〜―﹣+]\d+(?:話|巻|卷|篇|編)?|第?\d+\s*(?:巻|卷|話|篇|編)?|[IVXⅰⅴⅵⅶⅷⅸⅹ]+(?:[\-~～—〜―﹣+][IVXⅰⅴⅵⅶⅷⅸⅹ]+)?)\s*$/i, "")
              // ✅ 删除空格到篇/編/巻/卷/話结尾的内容
              .replace(/\s+[^ ]*(?:篇|編|巻|卷|話)$/, "")
              // ✅ 仅在末尾去除中点符号
              .replace(/[・･·•]+$/g, "")
